...
for (class in unique(Soybean$Class)) {
  print(
    paste0(class,": ",
      round(sum(is.na(Soybean[Soybean$Class==class,]))/nrow(Soybean[Soybean$Class==class,]),2),
      "%"
    )
  )
}
...
For higher $\phi_1$, the trend in the series is stronger since the new value depends more on the last value. 
...
The higher $\theta_1$, the higher the fluctuation is the time series. The new value depends more on the past errors.
...
An ARIMA(0,1,1) with drift is select. $\theta_1=0.3006$ and $\mu=0.1735$.
...
The residuals are independent with mean near 0. The residuals look like white noise.

```{r}
autoplot(austa_arima)
```

The forecasts of the next 10 periods look plausible with an increasing trend.
